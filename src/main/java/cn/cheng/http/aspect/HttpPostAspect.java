...
import cn.cheng.http.annotation.Post;
...
public class HttpPostAspect {
    public static final Logger logger = LoggerFactory.getLogger(HttpPostAspect.class);
    @Resource
    private WebClient webClient;

    @Pointcut("@annotation(cn.cheng.http.annotation.Post)")
    ...
    public Object beforePointcut(ProceedingJoinPoint joinPoint) {
        ...
        String url = post.url();
        logger.info("请求:{},方法：Post", url);
        String[] heads = post.heads();
        String body = post.body();
        String[] cookies = post.cookies();
        try {
